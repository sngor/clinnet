...
from src.handlers.users.disable_user import lambda_handler
...
class TestDisableUser:
    def test_disable_user_successful(self, lambda_environment, mock_aws_resources, test_user):
        cognito, user_pool_id = mock_aws_resources
        ...
        assert body["message"] == f"User {test_user} disabled successfully"

        # Verify user is actually disabled in Cognito
        cognito_user = cognito.admin_get_user(UserPoolId=user_pool_id, Username=test_user)
        assert cognito_user["Enabled"] is False

    def test_disable_user_already_disabled(self, lambda_environment, mock_aws_resources, test_user):
        cognito, user_pool_id = mock_aws_resources

        # First, disable the user
        cognito.admin_disable_user(UserPoolId=user_pool_id, Username=test_user)
        ...
        assert response["statusCode"] == 200
        assert f"User {test_user} disabled successfully" in json.loads(response["body"])["message"]

        # Verify the user remains disabled
        cognito_user = cognito.admin_get_user(UserPoolId=user_pool_id, Username=test_user)
        assert cognito_user["Enabled"] is False

    def test_disable_user_not_found(self, lambda_environment):
    ...
    @patch('boto3.client')
    def test_cognito_disable_failure(self, mock_boto3_client, lambda_environment, test_user):
        mock_cognito = mock_boto3_client.return_value
        mock_cognito.admin_disable_user.side_effect = Exception("Simulated Cognito Disabling Error")
        ...
        assert "Simulated Cognito Disabling Error" in body["message"]
