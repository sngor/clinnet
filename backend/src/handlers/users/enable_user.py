...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.info(f"Attempting to enable user: {username} in user pool {user_pool_id}")
        ...
        logger.info(f"Successfully enabled user: {username}")
        return add_cors_headers(response, request_origin)
    
    except ClientError as ce:
        logger.error(f"AWS ClientError enabling user '{username}': {ce}")
        return handle_exception(ce, request_origin)
    except Exception as e:
        logger.error(f"Unexpected error enabling user '{username}': {e}", exc_info=True)
