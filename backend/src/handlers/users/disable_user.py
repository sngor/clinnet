...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.info(f"Attempting to disable user: {username} in user pool {user_pool_id}")
        ...
        logger.error(f"AWS ClientError disabling user '{username}': {ce}")
        return handle_exception(ce, request_origin)
    except Exception as e:
        logger.error(f"Unexpected error disabling user '{username}': {e}", exc_info=True)
