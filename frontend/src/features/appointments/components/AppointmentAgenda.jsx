...
function AppointmentAgenda() {
  ...
  const getAppointmentStatus = (timeStr) => {
    ...
    if (hours < currentHour || (hours === currentHour && minutes < currentMinute)) {
      return 'past';
    } else if (hours === currentHour && Math.abs(minutes - currentMinute) <= 15) {
      return 'current';
    } else {
      return 'upcoming';
    }
              ...
              <React.Fragment key={appointment.id}>
                <ListItem 
                  ...
                  sx={{ 
                    py: 1.5,
                    px: 1,
                    borderLeft: timeStatus === 'current' ? `4px solid ${theme.palette.warning.main}` : 'none',
                    bgcolor: timeStatus === 'current' ? 'rgba(255, 152, 0, 0.05)' : 'transparent'
                  }}
                >
                  <Box sx={{ 
                    display: 'flex', 
                    ...
                    <Typography 
                      variant="body2" 
                      color={timeStatus === 'past' ? 'text.disabled' : 'text.primary'}
                      fontWeight="medium"
                    >
                      {appointment.time}
                    </Typography>
                  ...
                  <ListItemText
                    ...
                    secondary={
                      <Typography
                        variant="body2"
                        color={timeStatus === 'past' ? 'text.disabled' : 'text.secondary'}
                      >
                        {appointment.type}
                      </Typography>
                    }
                  ...
                  <Avatar 
                    sx={{ 
                      width: 32, 
                      height: 32,
                      bgcolor: timeStatus === 'past' ? 'grey.300' : 'primary.main',
