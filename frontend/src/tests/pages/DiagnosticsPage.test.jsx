...
describe('DiagnosticsPage', () => {
  ...
  describe('Automatic Checks on Page Load', () => {
    test('calls test functions for all testable services on initial render', async () => {
      ...
      const patientDataCard = screen.getByText('Patient Data (DynamoDB)').closest('div[class*="MuiCard-root"]');
      expect(await rtlWithin(patientDataCard).findByText('Online')).toBeInTheDocument();
    });
    ...
    test('shows "Checking..." status for testable services during initial load', async () => {
      ...
      adminService.checkS3Connectivity.mockImplementationOnce(() => new Promise(resolve => setTimeout(() => resolve({ success: true, message: 'Slow S3 OK' }), 100)));
      ...
      expect(rtlWithin(s3Card).getByText((content, element) => {
        return element.tagName.toLowerCase() === 'span' && content.startsWith('Checking...');
      })).toBeInTheDocument();
      
      // Wait for it to complete
      expect(await rtlWithin(s3Card).findByText('Online')).toBeInTheDocument();
      expect(await rtlWithin(s3Card).findByText('Slow S3 OK', {}, { timeout: 200 })).toBeInTheDocument(); // Ensure details are updated
    });
  });
  ...
  describe('S3 Service Testing (Manual after auto-check)', () => {
    test('successfully re-tests S3 connection and updates status', async () => {
      ...
      expect(await rtlWithin(s3Card).findByText('Online')).toBeInTheDocument();
      expect(rtlWithin(s3Card).getByText('S3 connection successful.')).toBeInTheDocument(); // From default mock in beforeEach
    ...
    test('handles S3 connection failure and updates status to Error', async () => {
      ...
      fireEvent.click(expandButton);
      expect(await rtlWithin(s3Card).findByText('S3 Manual Re-test Failed')).toBeInTheDocument();
    });
  });
  ...
  describe('Patient Data (DynamoDB) Service Testing (Manual after auto-check)', () => {
    ...
    test('successfully re-tests all CRUD operations and updates status', async () => {
      ...
      const initialCallCountForPatientData = adminService.checkDynamoDBCrud.mock.calls.filter(call => call[0] === apiId).length;
      ...
      adminService.checkDynamoDBCrud.mockImplementationOnce(async (id) => {
        if (id === apiId) return { create: 'OK', read: 'OK', update: 'OK', delete: 'OK (Manual)' };
    ...
    test('handles all CRUD operations failing and updates status to Offline', async () => {
      ...
      fireEvent.click(expandButton);
      expect(await rtlWithin(serviceCard).findByText('All operations successful.')).toBeInTheDocument();
      expect(rtlWithin(serviceCard).getByText(/D: OK \(Manual\)/)).toBeInTheDocument();
    });
  ...
  describe('User Data (Cognito) Service Testing (Manual after auto-check)', () => {
    ...
    test('successfully re-tests all Cognito CRUD operations and updates status', async () => {
      ...
      expect(await rtlWithin(serviceCard).findByText('Online')).toBeInTheDocument(); // After auto-check
      const initialCallCount = adminService.checkCognitoUsersCrud.mock.calls.length;

      adminService.checkCognitoUsersCrud.mockResolvedValueOnce({
        create: 'OK', read: 'OK (Manual)', update: 'OK', delete: 'OK (cleaned up)',
      });
      ...
      expect(await rtlWithin(serviceCard).findByText('Online')).toBeInTheDocument();
      expect(adminService.checkCognitoUsersCrud).toHaveBeenCalledTimes(initialCallCount + 1);
      
      const expandButton = rtlWithin(serviceCard).getByRole('button', { name: /expand/i });
      fireEvent.click(expandButton);
      expect(await rtlWithin(serviceCard).findByText(/R: OK \(Manual\)/)).toBeInTheDocument();
    });
  ...
  describe('Periodic Refresh Functionality', () => {
    ...
    test('enabling auto-refresh triggers tests periodically', async () => {
      ...
      await waitFor(() => expect(adminService.checkS3Connectivity).toHaveBeenCalledTimes(1));
      const s3InitialCalls = adminService.checkS3Connectivity.mock.calls.length;
      const dynamoInitialCalls = adminService.checkDynamoDBCrud.mock.calls.length;
      const cognitoInitialCalls = adminService.checkCognitoUsersCrud.mock.calls.length;
      ...
      await waitFor(() => { // Wait for async operations triggered by interval
        expect(adminService.checkS3Connectivity).toHaveBeenCalledTimes(s3InitialCalls + 1);
        // Each CRUD service is called again
        expect(adminService.checkDynamoDBCrud).toHaveBeenCalledTimes(dynamoInitialCalls + 3); // 3 dynamo services
        expect(adminService.checkCognitoUsersCrud).toHaveBeenCalledTimes(cognitoInitialCalls + 1);
      });

      // Advance time by another interval
      jest.advanceTimersByTime(AUTO_REFRESH_INTERVAL);
      await waitFor(() => {
        expect(adminService.checkS3Connectivity).toHaveBeenCalledTimes(s3InitialCalls + 2);
        expect(adminService.checkDynamoDBCrud).toHaveBeenCalledTimes(dynamoInitialCalls + 6);
        expect(adminService.checkCognitoUsersCrud).toHaveBeenCalledTimes(cognitoInitialCalls + 2);
      });
    });
    ...
    test('disabling auto-refresh stops periodic tests', async () => {
      ...
      jest.advanceTimersByTime(AUTO_REFRESH_INTERVAL);
      await waitFor(() => expect(adminService.checkS3Connectivity).toHaveBeenCalledTimes(s3InitialCalls + 1)); // After 1st refresh
    ...
    test('interval is cleared on unmount', async () => {
      ...
      jest.advanceTimersByTime(AUTO_REFRESH_INTERVAL);
      await waitFor(() => expect(adminService.checkS3Connectivity).toHaveBeenCalledTimes(s3InitialCalls + 1));
