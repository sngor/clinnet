...
// Use these functions to verify DynamoDB entity structure and log details

// Log DynamoDB entity details in the console
...
export function verifyDynamoStructure(entity, entityType = 'PATIENT') {
  ...
  const errors = [];
  // Required fields
  if (!entity.PK) { errors.push('Missing PK'); isValid = false; }
  if (!entity.SK) { errors.push('Missing SK'); isValid = false; }
  if (!entity.type) { errors.push('Missing type'); isValid = false; }
  // Entity-specific validation
  switch (entityType) {
    case 'PATIENT':
      if (!entity.id) { errors.push('Missing patient id'); isValid = false; }
      if (entity.PK && !entity.PK.startsWith('PAT#')) { errors.push(`PK should start with 'PAT#' but got '${entity.PK}'`); isValid = false; }
      if (!entity.GSI1PK || !entity.GSI1SK) { errors.push('Missing GSI1PK/GSI1SK for patient'); isValid = false; }
      break;
    case 'APPOINTMENT':
      if (!entity.id) { errors.push('Missing appointment id'); isValid = false; }
      if (entity.PK && !entity.PK.startsWith('APPT#')) { errors.push(`PK should start with 'APPT#' but got '${entity.PK}'`); isValid = false; }
      break;
    default:
      // Generic validation
  }
...
if (import.meta.env.DEV) {
  ...
  console.log('%cDynamoDB Debug Tools Available', 'background: #0066CC; color: white; padding: 4px 8px; border-radius: 4px; font-weight: bold');
  console.log('%cUse window.dynamoDb.logEntity(entity) to inspect DynamoDB entities', 'color: #0066CC');
  console.log('%cUse window.dynamoDb.verify(entity, "PATIENT") to validate DynamoDB entities', 'color: #0066CC');
}
