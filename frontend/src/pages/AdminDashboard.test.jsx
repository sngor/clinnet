...
describe('AdminDashboard', () => {
  ...
  test('renders without crashing', () => {
    ...
    expect(screen.getByText(/Here's what's happening in your clinic today/i)).toBeInTheDocument();
  });
  ...
  test('fetches data and displays correct card values', async () => {
    ...
    await waitFor(() => {
      const usersCard = screen.getByText('Users').closest('div[class*="MuiPaper-root"]'); // Find card by title
    ...
    const doctorsCount = mockUsers.filter(user => user.role === 'doctor').length;
    const doctorsCard = screen.getByText('Doctors').closest('div[class*="MuiPaper-root"]');
    ...
    const patientsCard = screen.getByText('Patients').closest('div[class*="MuiPaper-root"]');
    expect(patientsCard).toBeInTheDocument();
    expect(screen.getByText(mockPatientsList.length.toString())).toBeInTheDocument();
    
    // Check for Appointments card
    const appointmentsCard = screen.getByText('Appointments').closest('div[class*="MuiPaper-root"]');
    expect(appointmentsCard).toBeInTheDocument();
    expect(screen.getByText(mockAppointmentsList.length.toString())).toBeInTheDocument();
  ...
  test('handles API errors gracefully', async () => {
    adminService.listUsers.mockRejectedValue(new Error('Failed to fetch users'));
    ...
    await waitFor(() => {
      expect(screen.getByText(/Failed to load dashboard data: Failed to fetch users/i)).toBeInTheDocument();
    });

    // Ensure card values are 0 or reflect no data
    const usersCard = screen.getByText('Users').closest('div[class*="MuiPaper-root"]');
    expect(usersCard).toBeInTheDocument();
    ...
    const doctorCardValue = within(screen.getByText('Doctors').closest('div[class*="MuiPaper-root"]')).getByText('0');
    expect(doctorCardValue).toBeInTheDocument();
  });
});

// Helper to query within a specific element, if not using testing-library's built-in 'within'
// For example: const { getByText: getByTextInCard } = within(usersCard);
// This is already available from @testing-library/react, so just import it:
// import { render, screen, waitFor, within } from '@testing-library/react';
// The current implementation uses a more verbose way: screen.getByText('Users').closest(...) then screen.getByText(...)
// which can be error prone if the value (e.g. "0") is not unique.
// The above test 'fetches data and displays correct card values' correctly finds the cards first.
// The error test is refined to use 'within' for more precise value checking.

import { within } from '@testing-library/react'; // ensure within is imported for the error test
