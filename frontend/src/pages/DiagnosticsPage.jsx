...
const DiagnosticsPage = () => {
  ...
  const handleTestService = useCallback(async (serviceId) => {
    ...
    try {
      let response;
      if (currentServiceToTest.apiId === 's3' || currentServiceToTest.apiId === 'cognito_users') {
        response = await currentServiceToTest.testFunction();
      } else {
        response = await currentServiceToTest.testFunction(currentServiceToTest.apiId);
      }
      ...
      setServices(prevServices => {
          ...
          if (s.id === serviceId) {
            if (s.isCrudService) {
              ...
              if (response && response.success) {
                ...
                return { ...s, status: 'Error', details: response?.message || `${currentServiceToTest.name} test failed.` };
              }
            }
          }
          return s;
        ...
        const anyOffline = testableServices.some(s => s.status === 'Offline');
        ...
        return servicesAfterTestUpdate.map(s => {
          if (s.id === 'apiGateway' || s.id === 'lambdas') {
            if (anyOffline) return { ...s, status: 'Error', details: 'Critical dependency offline. Gateway/Lambdas likely non-functional.' };
      ...
      setServices(prevServices => {
        ...
        let afterCatchServices = prevServices.map(s =>
          s.id === serviceFailedId ? { ...s, status: 'Error', details: error.message || `An unknown error occurred testing ${currentServiceToTest.name}.` } : s
        );

        const failedServiceForGatewayCheck = prevServices.find(s => s.id === serviceFailedId && s.testable);
          ...
          afterCatchServices = afterCatchServices.map(s_ => {
            if (s_.id === 'apiGateway' || s_.id === 'lambdas') {
              if (s_.status !== 'Error' && s_.status !== 'Offline') {
                return { ...s_, status: 'Error', details: `Test for ${failedServiceForGatewayCheck.name} failed directly, impacting API/Lambda status.` };
              }
            }
            return s_;
          });
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        ...
        <FormControlLabel
          control={<Switch checked={isAutoRefreshEnabled} onChange={handleAutoRefreshToggle} />}
