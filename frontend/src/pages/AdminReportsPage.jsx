      ...
      return tabValue === 0 ? (
        ...
        <ResponsiveContainer width="100%" height={400}>
          ...
          >
            ...
            <Tooltip />
            <Legend />
            <Line type="monotone" dataKey="total" name="Total Appointments" stroke="#8884d8" activeDot={{ r: 8 }} />
          </LineChart>
        </ResponsiveContainer>
      );
      ...
      return tabValue === 0 ? (
        ...
        <ResponsiveContainer width="100%" height={400}>
          ...
          >
            ...
            <Tooltip formatter={(value) => [`$${value}`, 'Revenue']} />
            <Legend />
            <Line type="monotone" dataKey="revenue" name="Revenue" stroke="#2196f3" activeDot={{ r: 8 }} />
          </LineChart>
        </ResponsiveContainer>
      );
  ...
  return (
    <PageContainer>
      ...
      <Grid container spacing={3}>
          ...
          <ContentCard>
            ...
            <Box sx={{ borderBottom: 1, borderColor: 'divider', mb: 3 }}>
              <Tabs value={tabValue} onChange={handleTabChange} aria-label="chart type tabs">
                <Tab label={reportType === 'patients' ? "Pie Chart" : "Bar Chart"} />
                <Tab label={reportType === 'patients' ? "Bar Chart" : "Line Chart"} />
              </Tabs>
            </Box>
        ...
        <Grid item xs={12}>
          <ContentCard title="Report Summary">
            {loading ? (
              ...
              <Grid container spacing={3}>
                  ...
                  <Box sx={{ p: 2, textAlign: 'center' }}>
                    ...
                    <Typography variant="h3" color="primary" sx={{ my: 1 }}>
                      {reportType === 'appointments' ? 
                        reportData.appointments.reduce((sum, item) => sum + item.total, 0) : 
                        '509'}
                    </Typography>
                ...
                <Grid item xs={12} md={4}>
                  <Box sx={{ p: 2, textAlign: 'center' }}>
                    ...
                    <Typography variant="h3" color="primary" sx={{ my: 1 }}>
                      ${reportType === 'revenue' ? 
                        reportData.revenue.reduce((sum, item) => sum + item.revenue, 0).toLocaleString() : 
                        '37,700'}
                    </Typography>
                    <Typography variant="body2" color="text.secondary">
                      {timeRange === 'month' ? 'Last 30 days' : 
                ...
                <Grid item xs={12} md={4}>
                  <Box sx={{ p: 2, textAlign: 'center' }}>
                    ...
                    <Typography variant="h3" color="primary" sx={{ my: 1 }}>
                      {reportType === 'patients' ? 
                        reportData.patients.reduce((sum, item) => sum + item.value, 0) : 
                        '190'}
                    </Typography>
                    <Typography variant="body2" color="text.secondary">
                      {timeRange === 'month' ? 'Last 30 days' : 
