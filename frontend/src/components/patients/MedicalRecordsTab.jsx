...
function MedicalRecordsTab({ patientId }) {
  ...
  const fetchRecords = useCallback(async () => {
    ...
    setError(null);
    try {
      const fetchedRecords = await medicalRecordService.getMedicalRecords('patient', patientId);
      setRecords(Array.isArray(fetchedRecords) ? fetchedRecords : []);
    } catch (err) {
      console.error(`Failed to fetch medical records for patient ${patientId}:`, err);
      ...
      if (createdRecord && createdRecord.reportId && selectedFiles.length > 0) {
        for (const file of selectedFiles) {
          try {
            ...
            setModalError(`Record created, but failed to upload image ${file.name}. Some images may not have been saved.`);
          }
        }
      }
      await fetchRecords();
  ...
  const columns = [
    ...
    {
      field: "reportContent", headerName: "Report Content", flex: 1, minWidth: 200,
      renderCell: (params) => (
        <Tooltip title={params.value || ""} placement="top-start">
          <Typography variant="body2" noWrap>{params.value ? params.value.substring(0, 70) + (params.value.length > 70 ? "..." : "") : "N/A"}</Typography>
        </Tooltip>
      ),
    },
    {
            ...
            {params.value.map((url, index) => (
              url ? (
                <a key={index} href={url} target="_blank" rel="noopener noreferrer">
                  <img src={url} alt={`Record Image ${index + 1}`} style={{ width: 30, height: 30, objectFit: "cover", borderRadius: "4px" }} />
                </a>
              ) : (
                <Tooltip key={index} title="Image not available"><BrokenImageIcon sx={{ width: 30, height: 30, color: 'grey.400' }} /></Tooltip>
              )
            ))}
          </Box>
        );
  ...
  if (!patientId) {
    return <Alert severity="warning">No patient selected. Please select a patient to view their medical records.</Alert>;
  }
  ...
  return (
    <Box sx={{ mt: 2 }}>
      ...
      {loading ? <LoadingIndicator message="Loading records..." />
       ...
       : (
        <Box sx={{ height: 400, width: '100%' }}>
          <DataGrid
            ...
            density="compact"
            components={{
              NoRowsOverlay: () => <EmptyState title="No Records Found" description="This patient has no medical records yet." icon={<MedicalInformationIcon/>}/>,
            }}
            sx={{ "& .MuiDataGrid-cell:focus, & .MuiDataGrid-cell:focus-within, & .MuiDataGrid-columnHeader:focus, & .MuiDataGrid-columnHeader:focus-within": { outline: "none" }}}
          />
        </Box>
      )}
      ...
      <Dialog open={openCreateModal} onClose={handleCloseCreateModal} maxWidth="sm" fullWidth>
        <DialogTitle>Create New Medical Record for Patient: {patientId}</DialogTitle>
        <DialogContent>
          {modalError && <Alert severity="error" sx={{ mb: 2 }}>{modalError}</Alert>}
          <TextField margin="dense" label="Report Content" type="text" fullWidth multiline rows={4} variant="outlined" value={currentReportContent} onChange={(e) => setCurrentReportContent(e.target.value)} sx={{ mb: 2 }} required/>
          ...
          {imagePreviews.length > 0 && (
            <ImageList sx={{ width: '100%', height: 'auto', maxHeight: 250 }} cols={3} rowHeight={80} gap={8}>
              {imagePreviews.map((preview, index) => (
                <ImageListItem key={index} sx={{border: '1px solid #ddd', borderRadius: '4px'}}>
                  ...
                  <ImageListItemBar sx={{ background: 'rgba(0,0,0,0.7)'}} position="top" actionIcon={<IconButton sx={{ color: 'white' }} onClick={() => removeSelectedFile(index)} size="small"><CancelIcon fontSize="small"/></IconButton>} actionPosition="right" />
                </ImageListItem>
              ))}
            </ImageList>
          )}
        ...
        <DialogActions sx={{p: 2}}>
          ...
          <Button onClick={handleCreateRecord} variant="contained" color="primary" disabled={isSubmitting || !currentReportContent}>
        ...
        <Dialog open={openEditModal} onClose={handleCloseEditModal} maxWidth="sm" fullWidth>
          ...
          <DialogContent>
            ...
            {selectedRecord.imagePresignedUrls && selectedRecord.imagePresignedUrls.length > 0 ? (
                 <ImageList sx={{ width: '100%', height: 'auto', maxHeight: 120, mb:2 }} cols={5} rowHeight={60} gap={8}>
      ...
      <Dialog open={openDeleteConfirmDialog} onClose={handleCloseDeleteConfirmDialog}>
        ...
        <DialogContent>
          <Typography>
            ...
            {recordToDelete && ` (ID: ${recordToDelete.reportId})`} This action cannot be undone.
          </Typography>
        </DialogContent>
        <DialogActions sx={{p:2}}>
          <Button onClick={handleCloseDeleteConfirmDialog} color="secondary" disabled={isDeleting}>Cancel</Button>
